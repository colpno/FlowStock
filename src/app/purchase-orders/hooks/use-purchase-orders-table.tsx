import type { PurchaseOrdersTableDef } from "../components/purchase-orders-table";
import type { PurchaseOrder } from "@/types/data";
...
export const usePurchaseOrdersTable = () => {
  const [openDialog, setOpenDialog] = useState<null | "edit-status" | "remove-alert" | "edit-form">(
    null
  );
  const [editData, setEditData] = useState<null | PurchaseOrdersTableDef>(null);
  const columnHelper = createColumnHelper<PurchaseOrdersTableDef>();

  const handleEditClick = (defaultValues: PurchaseOrdersTableDef): void => {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor("supplier_id", {
        ...
        enableSorting: true,
        enableHiding: true,
        cell: (props) => props.row.original.supplier_name,
      }),
